...
function initialize_model(  ;n_particles = 10,
...
function agent_step!(particle,model)
    ...
    if rand()<0.1
        ...
        dist = particle.pos .- Tuple([(model.worldsize)/2 (model.worldsize)/2])
        model.globaldist[particle.id,:] = [dist[1] dist[2]]' #ändern
        model.movmentstate[particle.id,:] .= 1
        #println(model.movmentstate)
        #println("")
    end
...
function model_step!(model)
    if all(>(0),model.movmentstate)
        model.tick += 1
        model.meadist = sqrt((mean(model.globaldist,dims=1)[1])^2+(mean(model.globaldist,dims=1)[2])^2);
        println(model.meadist)
        model.movmentstate = zeros(10,1)
