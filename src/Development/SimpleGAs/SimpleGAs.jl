...
function recombine!( sga::SimpleGA, fitnesses=nothing)
	...
	for m in 1:nMatings
		...
		# To do: Now transfer the second part of daddy to sally, and the second
		# part of mummy to billy:
		# ???										# Sally also takes after daddy
		# ???										# Billy also takes after mummy

		# To do: Now store sally as m-th progeny and billy as (m+nMatings)-th progeny:
		# ???
		# ???
	end
...
function mutate!( sga)
	if sga.mu > 0
		...
		# You need to implement the new Casinos.draw() method to run this next line of code:
